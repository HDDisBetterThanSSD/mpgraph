...
void gather_my_gather(graph*, float*, unsigned int*, unsigned int*, unsigned int, unsigned int);
...
//////////////////////////////////////////
    ...
    for (unsigned int i=0; i<NUM_BINS; i++)
        {
        //rand();
        gather_my_gather(&G, updateBins[i], destIdBins[i], outDeg, i, destIdBinAddrOffset[NUM_BINS][i]);
        //myFunction();
        }
    end_gather();
...
void gather_my_gather(graph* G, float* updateBin, unsigned int* destIdBin, unsigned int* outDeg, unsigned int binId, unsigned int binSize)
{
