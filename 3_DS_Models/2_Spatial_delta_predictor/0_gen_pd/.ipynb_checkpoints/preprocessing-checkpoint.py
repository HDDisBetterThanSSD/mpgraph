...
def read_load_trace_data(load_trace, num_prefetch_warmup_instructions,num_total_instructions,skipping=0):
    ...
    def process_line(line):
        ...
        return int(split[0]), int(split[1], 16),  split[2], int(split[3], 16), int(float(split[4])), int(float(split[5]))
...
def read_load_trace_data(load_trace_train, NUM=-1):
    data=pd.read_csv(load_trace_train, sep=" ",header=None,index_col=None,names=["id","ip","r/w","addr","thread","phase"])
    #return train_data[:], eval_data[-TEST_NUM:]
    return data[:NUM]
...
def preprocessing(df):
    ...
    df["ip"]=df.apply(lambda x: to_hex(x['ip']),axis=1)
    df["addr"]=df.apply(lambda x: to_hex(x['addr']),axis=1)
    df['raw']=df['addr']
    ...
    df=df.dropna()
    return df[['id', 'ip', 'r/w', 'addr', 'thread', 'phase', 'block_address',
       'page_address', 'block_index','past','past_ip','past_page','future']]

    #return df[['id', 'addr', 'ip', 'raw', 'block_address',
    #   'page_address', 'page_offset', 'block_index', 'block_addr_delta',
    #   'patch','past','past_ip','past_page','future_delta','future']]

# [['id','ip','block_address', 'page_address', 'future_delta','future']]
#%%
'''
#file_path="/home/pengmiao/Disk/work/HPCA/ML-DPC-S0/LoadTraces/spec17/654.roms-s0.txt.xz"
file_path_train="/home/pengmiao/Disk/work/data/Graph_dataset/pr_block/wiki_block.champsim.trace.2.train_gather"

#file_path_test="/home/pengmiao/Disk/work/data/Graph_dataset/pr_block/wiki_block.champsim.trace.2.test_gather"

TRAIN_NUM = 1000
TEST_NUM=1000
SKIP_NUM=0

data = read_load_trace_data(file_path_train,TRAIN_NUM)
