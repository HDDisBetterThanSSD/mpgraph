...
def preprocessing_gen(data):
    ...
    past_page_name.append('page_address')
    #Pem, update done
    #past_name: ['patch_past_8', 'patch_past_7', 'patch_past_6', 'patch_past_5', 'patch_past_4', 'patch_past_3', 'patch_past_2', 'patch_past_1', 'patch']
    df["past"]=df[past_name].values.tolist()
    df["past_ip_abs"]=df[past_ip_name].values.tolist()
    df["past_page_abs"]=df[past_page_name].values.tolist()
...
def preprocessing_phase(data,phase):
    print("preprocessing")
    df=pd.DataFrame(data)
    df.columns=["id", "cycle", "addr", "ip", "hit"]
    df['raw']=df['addr']
    ...
    df["past"]=df[past_name].values.tolist()
    df["past_ip"]=df[past_ip_name].values.tolist()
    df["past_page"]=df[past_page_name].values.tolist()
    ...
    return df[['id', 'cycle', 'addr', 'ip', 'hit', 'raw', 'block_address',
       'page_address', 'page_offset', 'block_index', 'block_addr_delta',
       'patch','past','past_ip','past_page','phase']]
